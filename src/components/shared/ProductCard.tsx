...
const ProductCard = ({ product }: { product: IProduct }) => {
  return (
    <Card className="p-4 justify-between border-stone-400 shadow-none transition-shadow duration-300 hover:shadow-[1px_1px_10px_1px_rgba(0,0,0,0.25)] relative">
      ...
      {product.discount > 0 && <Badge variant="destructive" className="absolute left-2 top-2">Sale {product.discount}%</Badge>}
      {product.is_new && <Badge variant="default" className="absolute right-2 top-2">New Item</Badge>}
